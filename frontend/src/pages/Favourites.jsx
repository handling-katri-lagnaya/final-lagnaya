...
const Favourites = () => {
  ...
  const toggleFavourite = (matchId) => {
    setFavourites((prev) => {
      const newFavourites = prev.filter((id) => id !== matchId);
  ...
  const favouriteMatches = allMatches.filter((match) =>
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-premium-cream to-premium-peach">
      ...
      <div className="relative container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-6 sm:mb-8">
          <h1 className="font-playfair text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-2 flex items-center gap-3">
            <Heart className="w-8 h-8 text-red-500 fill-red-500" />
            Favourite Matches
          </h1>
          <p className="text-sm sm:text-base text-muted-foreground">
            {favouriteMatches.length > 0
              ? `You have ${favouriteMatches.length} favourite ${
                  favouriteMatches.length === 1 ? "match" : "matches"
                }`
        ...
        {favouriteMatches.length === 0 ? (
          <Card className="p-12 text-center border-border/50 shadow-[var(--shadow-card)]">
            <Heart className="w-16 h-16 mx-auto mb-4 text-muted-foreground" />
            <h2 className="text-xl font-semibold mb-2 text-foreground">
              No Favourites Yet
            </h2>
            <p className="text-muted-foreground mb-6">
              Start adding matches to your favourites by clicking the heart icon
              on any profile card
            </p>
